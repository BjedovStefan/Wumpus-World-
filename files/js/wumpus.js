class Wumpus {
    ...
    move() {
        ...
        if (this.worldData[this.x][this.y].isGold) {
          ...
          $('#warnings').text('There is no solution for this world');
          this.gameOver = true;
    ...
    handMove(direction) {
        ...
        if(this.areWeInDangerSpace()){
             if(this.worldData[this.x][this.y].isBreeze)
              $('#warnings').text('Feeling the breeze..');
             else if(this.worldData[this.x][this.y].isStench)
               $('#warnings').text('Feeling the stench..');
             else if(this.worldData[this.x][this.y].isStench && this.worldData[this.x][this.y].isBreeze)
               $('#warnings').text('Feeling the stench and the breeze..');
        }
        else if(this.areWeInFreeSpace())
            $('#warnings').text('Safe space');
        else if (this.areWeInPitLoop()) {
            $('#warnings').text('Stuck in the loop');
        }


      if(this.point < -20){
        this.gameOver = true;
        $('#status').text('No solution for this world');
        $('#warnings').text('');
        this.worldData[this.x][this.y].unsetPlayer();
        this.worldData[1][1].setPlayer();
      }
    ...
    drawCell() {
        for (var i = 1; i <= 4; i++) {
            for (var j = 1; j <= 4; j++) {
                ...
                if (cell.isPlayer)
                    $(".mboard .cell" + cell.x + "" + cell.y).css("background", "url(files/img/robin-figure.png) no-repeat #d0d0d0");

                if (cell.isVisited)
                    $(".mboard .cell" + cell.x + "" + cell.y).css("background-color", "#999");
            }
        }
